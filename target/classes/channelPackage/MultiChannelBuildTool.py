...
src_empty_file = sys.argv[5]+'info/imoney.txt'
# 创建一个空文件（不存在则创建）
...
for src_apk in src_apks:
    ...
    for line in lines:
        ...
        target_channel_name = line.strip()
        if '功夫贷' in lines[line]:
            target_channel = lines[line].strip() + "&" + target_channel_name
        elif '小期贷' in lines[line]:
            target_channel = lines[line].strip() + "&" + target_channel_name
        else:
            target_channel = target_channel_name + "&" + lines[line].strip()
        #判断是否是现金超市的包
        if 'supermarket' in src_apk_name:
            target_channel = target_channel_name + "&" + lines[line].strip()
        # 拼接对应渠道号的apk
        if '功夫贷' in lines[line] or '小期贷' in lines[line] or '开薪贷' in lines[line]:
        ...
        if 'supermarket' in src_apk_name:
            #获取秘钥
            response = urllib2.urlopen('https://api.91xjcs.com/supermarket/app/license?appId='+target_channel_name+'&platform=android')
            hjson = json.loads(response.read())
            print hjson['data']
            # 将渠道公钥写入文件（不存在则创建）
            keyfile = open(src_empty_file, 'w')
            keyfile.write(hjson['data']);
            keyfile.close();

        zipped.write(src_empty_file, empty_channel_file)
        if 'supermarket' in src_apk_name:
            zipInfo = zipped.getinfo(empty_channel_file)
            zipInfo.comment=hjson['data']
        # 关闭zip流
        zipped.close()
